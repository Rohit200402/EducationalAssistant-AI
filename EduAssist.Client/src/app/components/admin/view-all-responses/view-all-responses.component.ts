...
export class ViewAllResponsesComponent implements OnInit {
  ...
  load(): void { this.loading = true; this.aiResponseService.getAll(this.currentPage, 5, this.search).subscribe({ next: (res) => { this.responses = res.items; this.totalPages = res.totalPages; this.totalCount = res.totalCount; this.loading = false; }, error: () => { this.loading = false; } }); }
