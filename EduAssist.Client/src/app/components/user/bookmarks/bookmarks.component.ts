...
export class BookmarksComponent implements OnInit {
  ...
  constructor(private bookmarkService: BookmarkService, private toast: ToastService) {}
  ngOnInit(): void { this.load(); }
  load(): void { this.loading = true; this.bookmarkService.getAll(this.currentPage, 5, this.search).subscribe({ next: (res) => { this.bookmarks = res.items; this.totalPages = res.totalPages; this.totalCount = res.totalCount; this.loading = false; }, error: () => { this.loading = false; } }); }
  ...
  delete(id: number): void { this.bookmarkService.delete(id).subscribe({ next: () => { this.toast.success('Bookmark removed!'); this.load(); }, error: () => this.toast.error('Failed') }); }
}
