...
export class MyRequestsComponent implements OnInit {
  ...
  load(): void { this.loading = true; this.requestService.getMyRequests(this.currentPage, 5, this.search).subscribe({ next: (res) => { this.requests = res.items; this.totalPages = res.totalPages; this.totalCount = res.totalCount; this.loading = false; }, error: () => { this.loading = false; } }); }
