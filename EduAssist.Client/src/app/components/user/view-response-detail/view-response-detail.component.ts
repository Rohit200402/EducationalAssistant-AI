...
export class ViewResponseDetailComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.requestService.getById(id).subscribe(r => { this.request = r; });
    this.aiResponseService.getByRequest(id).subscribe({ next: (res) => { this.responses = res; this.loading = false; }, error: () => { this.loading = false; } });
  }
  regenerate(): void {
    if (!this.request) return;
    this.regenerating = true;
    this.requestService.regenerate(this.request.userRequestId).subscribe({
      next: (res) => { this.responses.unshift({ aiResponseId: res.aiResponseId, response: res.response, createdAt: res.createdAt, userRequestId: this.request!.userRequestId }); this.regenerating = false; this.toast.success('New response generated!'); },
